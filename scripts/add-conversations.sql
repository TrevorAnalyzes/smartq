-- Add sample conversations for TechCorp UK and HealthPlus Medical Services
-- Run this SQL script directly in your Supabase SQL editor

-- First, let's check if we have agents for these organizations
-- You'll need to replace the agent IDs below with actual agent IDs from your database

-- Sample conversations for TechCorp UK (org-techcorp-uk)
...
SELECT
  gen_random_uuid(),
  (SELECT id FROM "VoiceAgent" WHERE "organizationId" = 'org-techcorp-uk' LIMIT 1),
  'org-techcorp-uk',
  '+447900' || lpad((random() * 1000000)::int::text, 6, '0'),
  (ARRAY['John Smith', 'Emma Wilson', 'Oliver Brown', 'Sophie Taylor', 'James Davis'])[floor(random() * 5 + 1)],
  ...
  (ARRAY['Product inquiry', 'Technical support', 'Billing question', 'Order status', 'Complaint resolution'])[floor(random() * 5 + 1)],
